...
(define-data-var governanceTokenPrincipal principal .mega-token)
...
(map-set parameters "voteThreshold" u1)
(map-set parameters "quorumThreshold" u12500)
...
(define-read-only (get-voting-power (voter principal) (blockHeight uint))
  (at-block
    (unwrap! (get-block-info? id-header-hash blockHeight) none) (some (unwrap-panic (contract-call? .mega-token get-balance voter)))
  )
)
...
(define-public (vote (for bool) (proposal principal) (delegator (optional principal)))
	(let
		...
		(asserts! (unwrap-panic (can-vote voter (try! (get-parameter "voteThreshold")) .mega-token)) ERR_INSUFFICIENT_WEIGHT)
		(asserts! (is-eq u0 (get-current-total-votes proposal voter .mega-token)) ERR_ALREADY_VOTED)
		(map-set MemberTotalVotes {proposal: proposal, voter: voter, governanceToken: .mega-token}
			(+ (get-current-total-votes proposal voter .mega-token) amount)
		)
